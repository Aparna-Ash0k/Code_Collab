...
// Import schema harmonization utilities
const { SchemaHarmonizer } = require('../client/src/utils/SchemaHarmonizer');
...
class DatabaseMigration {
  constructor() {
    ...
    this.schemaHarmonizer = new SchemaHarmonizer();
  }
...
.App-header {
  ...
  /**
   * Validate schema compatibility between PostgreSQL and Firebase
   */
  async validateSchemaCompatibility() {
    console.log('🔍 Validating schema compatibility...');

    const entityTypes = ['users', 'projects', 'files'];
    const incompatibilities = [];

    for (const entityType of entityTypes) {
      try {
        // Get sample data from both sources if available
        let postgresData = null;
        let firebaseData = null;

        if (this.postgres) {
          const tableName = entityType === 'chatMessages' ? 'chat_messages' : 
                           entityType === 'fileVersions' ? 'file_versions' :
                           entityType === 'collaborationSessions' ? 'collaboration_sessions' : entityType;
          
          const result = await this.postgres.query(`SELECT * FROM ${tableName} LIMIT 1`);
          postgresData = result.rows[0];
        }

        if (this.firebase.db) {
          const snapshot = await this.firebase.db.collection(entityType).limit(1).get();
          if (!snapshot.empty) {
            firebaseData = snapshot.docs[0].data();
          }
        }

        if (postgresData && firebaseData) {
          const compatibility = await this.schemaHarmonizer.checkSchemaCompatibility(
            postgresData, 
            firebaseData, 
            entityType
          );
        ...
        console.warn(`Could not validate schema for ${entityType}:`, error.message);
      }
    }

    if (incompatibilities.length > 0) {
      console.warn('⚠️ Schema incompatibilities detected:');
      incompatibilities.forEach(item => {
        console.warn(`  ${item.entityType}:`, item.conflicts);
      });

      // Generate migration scripts for incompatibilities
      for (const item of incompatibilities) {
        const script = this.schemaHarmonizer.generateMigrationScript(item.conflicts);
        console.log(`📝 Migration script for ${item.entityType}:`, script);
      }
    } else {
