...
export class SocketAdapter extends EventEmitter {
  ...
  setupSocketListeners() {
    ...
    this.socket.on('file-operation-broadcast', (operation) => {
      ...
      // Debug: Log user IDs for echo prevention
      console.log('🔍 Echo check: operation.userId =', operation.userId, 'vs socket.userId =', this.socket.userId);
      
      // Fixed echo prevention - only skip if both values are defined and equal
      if (operation.userId && this.socket.userId && operation.userId === this.socket.userId) {
        console.log('🔄 Ignoring echo from own operation');
        return; // Skip own operations to prevent echo
      }
      
      console.log('✅ Processing remote operation from different user');
      
      // Debug: Check if we're emitting the event
      console.log('📡 Emitting operation event for FileSystemManager:', operation);
      
      // Emit operation to FileSystemManager for processing
      // FileSystemManager expects 'operation' events from adapters
      this.emit('operation', operation);
      
      // Debug: Confirm event was emitted
      console.log('📤 operation event emitted to FileSystemManager');
    });
